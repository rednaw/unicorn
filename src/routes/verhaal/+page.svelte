<script lang="ts">
	...
	async function start() {
		...
		actEls.forEach((act) => {
			...
			// Per-act scroll length. Sized so the poem-reveal dwell gives roughly
			// one viewport of comfortable reading distance for the longest poems.
			...
			tl.to(drawingsEl[0], { autoAlpha: 1, scale: 1, duration: 0.5 })
				.to(drawingsEl[1], { autoAlpha: 1, scale: 1, duration: 0.5 }, '+=0.5')
				.to(drawingsEl[0], { autoAlpha: 0.35, duration: 0.4 }, '<')
				.to(poemEl, { autoAlpha: 1, y: 0, duration: 0.5 }, '+=0.15')
				// Dwell on the poem at full opacity — the act's "reading time".
				.to({}, { duration: 2 })
				.to([drawingsEl, poemEl, titleEl], { autoAlpha: 0, duration: 0.35 });
		});
