import { pickAudioSrc, vinylSrc } from '$lib/audio-format';
 ...
 * Vinyl drop/lift/overlap are baked into generated `*.vinyl.m4a` / `.webm` at encode.
 */
function createAudioPlayer() {
	...
	function playDrawing(drawingId: string, opts: { fromStart?: boolean } = {}): void {
		...
		const src = pickAudioSrc(vinylSrc(drawing.track.src));
